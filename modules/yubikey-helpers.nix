      ...
      ${lib.concatStringsSep "\n" (
        lib.mapAttrsToList (
          ...
          ''
            ...
            mkdir -p $(dirname "$KEYSDIR/${key}")
            ykman piv certificates export ${slot} "$KEYSDIR/${key}.x509.pem"
            echo "Generating public key for ${key} from extracted certificate"
            openssl x509 -in "$KEYSDIR/${cfg.avb.key}.x509.pem" -pubkey -noout > "$KEYSDIR/${cfg.avb.key}.pub"
          ''
        ) cfg.pkcs11.presets.yubikey-piv.slotMap
      )}

      # Generate AVB public key and PKMD.
      openssl x509 -in "$KEYSDIR/${cfg.avb.key}.x509.pem" -noout -pubkey > "$KEYSDIR/${cfg.avb.key}.pub"
      avbtool extract_public_key --key "$KEYSDIR/${cfg.avb.key}.pub" --output "$KEYSDIR/${cfg.avb.key}_pkmd.bin"
    '';
in
